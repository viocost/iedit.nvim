...
function M.start(ranges, config)
	...
	local old_text =
		table.concat(vim.api.nvim_buf_get_text(buf, ranges[1][1], ranges[1][2], ranges[1][3], ranges[1][4], {}), "\n")
	local b = vim.b[buf]
		...
		callback = function()
			...
			for _, mark_id in pairs(get()) do
				...
				M.set_extmark(buf, {
					...
					range[2],
					range[1] + #new_text - 1,
					(#new_text == 1 and range[2] or 0) + #new_text[#new_text],
...
function M.update_extmark_highlight(buf, mark_id, hl_group)
	local mark = vim.api.nvim_buf_get_extmark_by_id(buf, M.ns, mark_id, { details = true })
	if mark and mark[3] then
		vim.api.nvim_buf_set_extmark(buf, M.ns, mark[1], mark[2], {
			id = mark_id,
